...
class SpeechLLM(nn.Module):
    ...
    def __init__(self, config: SpeechLLMConfig):
        ...
        if torch.cuda.is_available():
            ...
            supports_low_precision = bool(mps_backend and torch.backends.mps.is_available())

        use_mixed_precision = config.mixed_precision and supports_low_precision
        if config.mixed_precision and not use_mixed_precision:
            warnings.warn("mixed_precision=True but no CUDA/MPS device found; falling back to float32.")
        ...
        self.config.mixed_precision = use_mixed_precision
    ...
    def encode_audio(
        ...
        whisper_outputs = self.whisper_encoder(
            audio=audio, attention_mask=attention_mask
        )
    ...
    def prepare_inputs_embeds(
        ...
        for b in range(batch_size):
            ...
            if audio_mask.any() and len(audio_positions) > b:
                ...
                mixed_emb = torch.cat(
                    [text_emb[:pos], audio_emb, text_emb[pos:]], dim=0
                )
        ...
        for emb in mixed_embeddings:
            ...
            if emb.shape[0] < max_len:
                ...
                padding = torch.zeros(
                    max_len - emb.shape[0], emb.shape[1], device=device, dtype=emb.dtype
                )
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if input_ids is not None:
                ...
                inputs_embeds = self.prepare_inputs_embeds(
                    input_ids=input_ids, audio_embeddings=audio_embeddings
                )
        ...
        if inputs_embeds is not None and self.config.use_gradient_checkpointing:

            # 當使用 gradient checkpointing 並傳入 inputs_embeds 時，確保其需要梯度以維持反向傳遞
            ...
            if rvq_labels is not None:
                ...
                for pred, target in zip(audio_outputs["rvq_predictions"], rvq_labels):
                    ...
                    rvq_loss += F.cross_entropy(
                        pred.view(-1, pred.size(-1)), target.view(-1), ignore_index=-100
                    )
    ...
    def generate_speech(
        ...
        with torch.no_grad():
            ...
            last_hidden_states = generated_ids.hidden_states[-1][
                -1
            ]  # 最後一層的最後一個狀態
...
if __name__ == "__main__":
    ...
    config = SpeechLLMConfig(
        llm_model_name="Qwen/Qwen2.5-7B-Instruct", use_lora=True, freeze_llm=True
    )
