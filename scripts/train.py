...
def main():
    ...
    print("配置載入完成:")
    print(f"  模型: {model_config.llm_model_name}")
    print(f"  輸出目錄: {training_config.output_dir}")
    print(f"  批次大小: {training_config.batch_size}")
    print(f"  學習率: {training_config.learning_rate}")
    print(f"  總 epochs: {training_config.num_epochs}")
    ...
    print(f"模型初始化完成，參數量: {model.get_trainable_parameters():,}")
    ...
    if os.path.exists(data_config['eval_data_file']):
        ...
        print(f"評估資料集載入完成: {len(eval_dataset)} 個樣本")
    else:
        print("未找到評估資料集，跳過評估")
    
    print(f"訓練資料集載入完成: {len(train_dataset)} 個樣本")
    ...
    if args.resume_from_checkpoint:
        print(f"從檢查點恢復訓練: {args.resume_from_checkpoint}")
