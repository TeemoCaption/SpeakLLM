...
- **特色**：
  ...
  - DTW 動態時間規劃
  - 拼音音節級對齊
  - 支援 jieba 和 pkuseg 分詞
  - Whisper 時間戳提取

#### `interleaving.py` - 交錯序列生成
- **功能**：生成語音-文字交錯序列（台灣口音專用）
- **特色**：
  - 音節級交錯標註
  - 4 層 RVQ 對齊策略
  - 支援多種對齊單位
...
- **功能**：處理語音資料集（台灣口音專用）
- **支援資料集**：
  - AISHELL-1/2/4（可適配台灣口音）
  - WenetSpeech（可適配台灣口音）
  - Common Voice zh-TW（台灣口音）
  - THCHS-30

### 5. 訓練系統 (`speechllm/training/`)

#### `trainer.py` - 三階段訓練器
- **Stage A**：台灣中文輸入對齊 + KL 蒸餾
- **Stage B**：台灣中文語音生成訓練
- **Stage C**：四種模式聯合訓練
- **台灣中文優化**：
  - 階層式 RVQ 損失權重
...
- **台灣中文配置**：語言設置、對齊策略、文字正規化、台灣口音專用支援
- **訓練配置**：三階段參數、損失權重、硬體設置
- **資料配置**：台灣口音資料集路徑、處理參數、快取設置
...
### 5. 自定義開發
- 參考 README.md 中的中文使用指南
- 查看各模組的詳細文檔
- 根據需求修改配置和代碼

這個專案結構設計確保了代碼的清晰性、可維護性和擴展性，特別針對台灣中文口音的語音對話場景進行了全面優化。
